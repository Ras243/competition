            ...
            <div class="content-wrapper" style="margin-top: 0px;">
                <div class="jumbotron jumbotron-fluid" style="height: auto; background-color: #37517e">
                    <div class="container-fluid text-center">
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center mt-0 text-white"><b>DAFTAR LOMBA</b></h3>
                                ...
                                <div class="card border-0 shadow-sm rounded">
                                    <div class="card-body mb-3 mt-3">
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <a href="{{ route('daftarLomba.create') }}"
                                                class="btn btn-md  mb-3 text-white btn-primary">Tambah Data
                                            </a>
                                        ...
                                        <table id="dataTable" class="table table-striped table-bordered">
                                            ...
                                            <tbody>
                                                @php
                                                    $no = 1;
                                                @endphp
                                                @forelse ($daftarLomba as $lomba)
                                                    <tr>
                                                        <td>{{ $no++ }}</td>
                                                        <td>{{ $lomba->nama_lomba }}</td>
                                                        <td class="text-center">
                                                            <img src="{{ asset('storage/lomba/' . $lomba->sampul_lomba) }}"
                                                                class="rounded"
                                                                style="width: 100px; height: 100px; object-fit: cover;">
                                                        </td>
                                                        <td>{{ $lomba->deskripsi_lomba }}</td>
                                                        <td> Rp.{{ $lomba->biaya_pendaftaran }}</td>
                                                        <td>{{ $lomba->batas_pendaftaran }}</td>
                                                        <td>
                                                            <div class="d-flex">
                                                                <a href="{{ route('daftarLomba.edit', $lomba->id) }}"
                                                                    class="btn btn-sm btn-primary mr-2"><i
                                                                        class="fas fa-edit">edit </i>
                                                                </a>
                                                                <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                                                    action="{{ route('daftarLomba.destroy', $lomba->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
